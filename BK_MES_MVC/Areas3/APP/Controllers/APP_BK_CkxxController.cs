...
namespace BK_MES_MVC.Areas.APP.Controllers
{
    public class APP_BK_CkxxController : Controller
    {
            ...
            try
            {
                 ...
                 */
                    ...
                    else if (mark.Equals("2"))
                    {
                        ...
                        /**
                            根据成品批号查询出该批号所在的订单头信息
                            str有值则表明该成品批号在其他订单中 在前台页面上弹出禁止 同时不查询显示该批号的信息
                            str无值则表明该成品批号并不被其他订单锁定 可以选择出  则返回前台该批号的信息 提示
                         */
                        DbRawSqlQuery<ddt> ddts = _rs.IQueryable<ddt>(@"select 订货头编号 as dhtbh from [BK_MES].[dbo].[订货明细表] B 
                                                                        inner join (select 成品库存编号,销售订单编号 from [BK_MES].[dbo].[成品库存锁定表] C 
												                        inner join (select 自动编号 from [BK_MES].[dbo].[成品信息表_4] 
                                                                        where 批号='"+ CP_barcode+"') A on c.成品库存编号=a.自动编号) D on d.销售订单编号=b.自动编号");
        ...
        /// <returns></returns>
        //public ActionResult CP_rk_message_select(string user)
        //{
