...
namespace BK_MES_MVC.Areas.APP.Controllers
{
    public class APP_BK_CkxxController : Controller
    {
        ...
        public ActionResult CP_rk_message_select(string _user)
        {
            ...
            try
            {
                ...
                DbRawSqlQuery<Select_CP_xx> list = _rs.IQueryable<Select_CP_xx>(@"select 
																	    ...
																	    where not exists(select 1 from [BK_MES].[dbo].[成品库存锁定表] G where G.成品库存编号=C.自动编号) and B.操作人编码 =" + _user+" and B.入库日期 between '"+dtime_before+"' and  '"+dtime_after+"'");
        ...
        /// <returns></returns>
        public ActionResult CP_rk_message_delete(string cp_zdbh) {
            ...
            try {
                ...
                SD.Data.IRepositoryBase _rs = new SD.Data.RepositoryBase();
                int _zdbh = Convert.ToInt32(cp_zdbh);
                var rkb_message =_rs.FindEntity<db_models.cprkb>(x=>x.cpxxbh==_zdbh);
                var _rksj = rkb_message.rkrq;
                var _kwh = rkb_message.kwh;
                var _rkfsm = rkb_message.rkfsm;
                var _rkr = rkb_message.czrbm;
                var _rkbz = rkb_message.bz;
                var _cxr = rkb_message.czrbm;
                _rs.BeginTrans();
                _rs.Delete<db_models.cprkb>(x => x.cpxxbh ==_zdbh );
                _rs.Delete<db_models.cpkwb>(x => x.cpbh ==_zdbh);
                _rs.Insert<db_models.cprkcxb>(new db_models.cprkcxb() { cpxxbh=_zdbh,rksj=_rksj,kwh =_kwh,rkfsm=_rkfsm,rkr=_rkr,rkbz =_rkbz,cxsj=DateTime.Now,cxyy="10",cxr=_cxr});
                _rs.Commit();
